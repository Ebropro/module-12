...
//seed studs
 if (!await context.Students.AnyAsync(ct))
        ...
        // Only seed if students exist (from the M5 seeder or existing data)
...
//         // ── Courses ───────────────────────────────────────────
//         // Idempotent: if any courses exist, skip entirely
//         if (!await context.Courses.AnyAsync(ct))
//         {
//             foreach (var (code, title, maxCapacity) in Courses)
//                 context.Courses.Add(new Course
...
//         }
        ...
        // ── Assessments ───────────────────────────────────────
        // Seed 2-3 assessments per course so GET /api/courses/{id}/assessments
        // returns real data immediately — no manual POST needed after every restart
        //
        // Idempotent: skip if any assessments already exist
