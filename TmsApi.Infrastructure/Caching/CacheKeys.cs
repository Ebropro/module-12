...
public static class CacheKeys
{
    ...
    // Old entries become unreachable on the next read — no manual flush needed.
    private const string SchemaVersion = "v2";

    public const string CoursesTag = "courses";

    // A single course by id (used by GET /api/courses/{id} and similar reads).
    public static string Course(int id) => $"{SchemaVersion}:course:{id}";

    // A page of the course list. 
